...
#+BEGIN_SRC emacs-lisp
 ...
 ;; Automatically tangle this  file when we saving it
...
#+begin_src emacs-lisp
  ...
  (cheatsheet-add :group 'Common
                  :key "C-x C-s"
                  :description "Save file in buffer")
  (cheatsheet-add :group 'Common
                  :key "C-x s"
                  :description "Save unsaved buffers in session")
  (cheatsheet-add :group 'Help
                  :key "M-x describe"
                  :description "Help on variables, modes etc")  
  (cheatsheet-add :group 'Help
                  :key "C-h m"
                  :description "Show major mode key bindings, minor modes enabled..")  
  (cheatsheet-add :group 'Help
                  :key "C-h v"
                  :description "Decribe variable")  
...
#+begin_src emacs-lisp
  (cheatsheet-add :group 'Custom
                  :key "M-x fac/..."
                  :description "Functions ac...")
  (cheatsheet-add :group 'Buffers
                  :key "C-c a [number]"
                  :description "Split frame in n buffers")  
...
#+begin_src emacs-lisp
  ...
  (defun fac/copy-current-line-position-to-clipboard ()
    "Copy current line in file to clipboard as '</path/to/file>::<line-number>'."
    (interactive)
    (let ((path-with-line-number
           (concat (buffer-file-name) "::" (number-to-string (line-number-at-pos)))))
  ...
  ;;(define-key global-map (kbd "C-c C-g C-l") 'fac/copy-current-line-position-to-clipboard)
...
** remove quotes
#+begin_src emacs-lisp
  (defun fac/remove-quotes-from-region (start end)
    "Remove surrounding double quotes from words in the selected region."
    (interactive "r")
    (save-excursion
      (goto-char start)
...
** Buffer layout templates
#+begin_src emacs-lisp
  (defun fac/split-window-three-columns ()
    "Split the current window into three columns."
    (interactive)
    ;; Ensure there is only one window to begin with
    (delete-other-windows)
    ;; Split the window twice into three columns
    (split-window-right)
    (split-window-right)
    ;; Adjust sizes if necessary
    (balance-windows)
    )  ; This ensures the windows are balanced equally

  (defun fac/split-window-six ()
    "Split the current window into six panes: three columns and two rows in each column."
    (interactive)
    ;; Ensure there is only one window to begin with
    (delete-other-windows)
    ;; First split into three columns
  ...
  (global-set-key (kbd "C-c a 3") 'fac/split-window-three-columns)
  (global-set-key (kbd "C-c a 4") 'fac/split-window-four)
  (global-set-key (kbd "C-c a 5") 'fac/split-window-fourr)  
  (global-set-key (kbd "C-c a 6") 'fac/split-window-six)
...
#+begin_src emacs-lisp
  (cheatsheet-add :group 'Navigation
                  :key "C-a"
                  :description "Go to first character of line")
  (cheatsheet-add :group 'Navigation
                  :key "C-a C-a"
                  :description "Go to beginning of line")
  (cheatsheet-add :group 'Navigation
                  :key "M-[+-]"
                  :description "Scroll up/down by 4")
  (cheatsheet-add :group 'Navigation
                  :key "M-g M-<up,down,left..>"
                  :description "Move buffers to other windows")
  (cheatsheet-add :group 'Navigation
                  :key "C-c C-g g"
                  :description "Search in google or look in selected region")
...
* UI Configuration
#+begin_src emacs-lisp
  (cheatsheet-add :group 'UI
                  :key "ztree-diff/dir"
                  :description "Diff 2 directories or navigate")
  (cheatsheet-add :group 'UI
                  :key "C-8 C-9 C-0"
                  :description "Transparency of buffers")
  (cheatsheet-add :group 'UI
                  :key "C-c C-g z"
...
#+begin_src emacs-lisp
  ...
  (global-set-key (kbd "C-c C-g z") 'counsel-load-theme)
...
** Contrib
set :ignore: in an org header such that the header is not included but the contents are.
...
To automatically build links to pdfs pages
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
  ...
  ;; (setq bibtex-completion-bibliography '("~/Documents/Engineering.bib")
  	;bibtex-completion-library-path '("~/Dropbox/emacs/bibliography/bibtex-pdfs/")
  	;bibtex-completion-notes-path "~/Dropbox/emacs/bibliography/notes/"
  	;;bibtex-completion-notes-template-multiple-files "* ${author-or-editor}, ${title}, ${journal}, (${year}) :${=type=}: \n\nSee [[cite:&${=key=}]]\n"
  ;;	)
  (setq ffap-file-name-with-spaces 1) ;; get file with spaces
  ...
  (org-roam-bibtex-mode)
   ...
   (use-package org-noter
     ...
     (setq
      ...
      org-noter-notes-search-path (list "~/RoamNotes/refs/")
...
use-package wont work, git clone to .emacs.d/repos
https://github.com/jkitchin/ox-ipynb
#+begin_src emacs-lisp
  ;; (use-package ox-ipynb
  ;;   )
  (add-to-list 'load-path "~/.emacs.d/repos/ox-ipynb")
  (require 'ox-ipynb)
...
#+begin_src emacs-lisp
  ...
  ;;cycle languages
  (setq-default ispell-program-name (string-trim (shell-command-to-string "which aspell")))
...
#+begin_src emacs-lisp
  ...
  ;;   ;; Enable automatic UI configuration
  ;;   :config
...
** Python
https://andrewfavia.dev/posts/emacs-as-python-ide/
...
doing workon inside the project folder and selecting the environment seems to work for now.

not working for automatically detecting environmet, maybe this helps
https://www.kotaweaver.com/blog/emacs-python-lsp/
#+begin_src emacs-lisp
  ;; workon home
  ;; (setenv "WORKON_HOME" "~/anaconda3/envs/")
  (setenv "WORKON_HOME" *py_env*)
  (defalias 'workon 'pyvenv-workon)

  ;; make shell interpreter ipython (better completion)
  ...
  (defun fac-toggle-python-shell-interpreter ()
    "Toggle `python-shell-interpreter` and `python-shell-interpreter-args` between `python` and `ipython`."
    (interactive)
    (if (string-equal python-shell-interpreter "python")
        (progn
          (setq python-shell-interpreter "ipython")
    ...
    (message "Switched to interpreter: %s with args: %s"
             python-shell-interpreter python-shell-interpreter-args))

  (define-key python-mode-map (kbd "C-c a p") 'fac-toggle-python-shell-interpreter)  
#+end_src
...
** Chatgpt
#+begin_src emacs-lisp
  (global-set-key (kbd "C-c C-g C-c") 'chatgpt-shell)  
#+end_src
